...
void MinerStatus::PushStatusNotification() const
{
    LOCK(m_cs);
...
double EfficiencyReport::StakingLoopEfficiency() const
{
    ...
    return masked_time_intervals_covered * 100.0 / (double)masked_time_intervals_elapsed;
}
...
void EfficiencyReport::UpdateMetrics(uint64_t weight_sum, int64_t balance_weight)
{
    ...
    const int64_t masked_time_elapsed = GRC::MaskStakeTime(GetTimeMillis() / 1000 + GetTimeOffset())
